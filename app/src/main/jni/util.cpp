//
// Created by qumiao on 2020/3/25.
//
#include "util.h"

class ifstream;


static const u8 BitReverseTable256[] = {
        
};

u8 reverseBit(u8 val) {
    return BitReverseTable256[val];
}

u32 bitFetch(u32 src, int offset, int count) {
    int i;
    u32 result = 0;
    for (i = 0; i < count; i++) {
        result |= src << (31 - offset - i) >> (31 - i);
    }
    return result;
}

u32 reverseByte(u32 src) {
    u32 result = 0;
    void *resultPtr = &result;
    void *srcPtr = &src;
    ((u8 *) resultPtr)[0] = ((u8 *) srcPtr)[3];
    ((u8 *) resultPtr)[1] = ((u8 *) srcPtr)[2];
    ((u8 *) resultPtr)[2] = ((u8 *) srcPtr)[1];
    ((u8 *) resultPtr)[3] = ((u8 *) srcPtr)[0];
    return result;
}

u32 computeEncryptLoopCount(const u8 *vector) {
//    u32 R7_F8[1];

//    *R7_F8 = (u32) vector;

    u32 v2 = 0x10;
//    u32 v0 = *R7_F8;
//    u32 v1 = *((u32 *) v0);
//    v0 = 0xCCCCCCCD;
//    u64 tmp = v1 * (u64) v0;
//    v0 = (u32) tmp;
//    v2 = (u32) (tmp >> 32);
//    //v0 = R7_68;
//    v2 = v2 >> 2;
//    v2 += v2 << 2;
//    v1 -= v2;
//    v2 = 0x20;
//    v1 = v2 + (v1 << 3);

    return v2;
}

void eorPart8(u8 *vector, u8 *content) {
    for (int i = 0; i < 8; i++) {
        content[i] = content[i] ^ vector[i];
    }
}

u8 *createBufferFromFile(const char *filePath, u32 &bufLen) {
    FILE *file = fopen(filePath, "rb");
    if (!file) {
        bufLen = 0;
        return NULL;
    }
    fseek(file, 0, SEEK_END);
    int length = ftell(file);
    rewind(file);

    bufLen = length;
    u8 *buffer = new u8[length];
    fread(buffer, 1, length, file);

    fclose(file);
    return buffer;
}